...
def main(args):
    ...
    # street option
    option_list = pd.read_csv(args.option_list)

    coords = pd.DataFrame(
        {"region": ["Sao Paulo - Brazil", "Quito - Ecuador", "Miraflores - Peru", "Montevideo - Uruguay"],
    ...
    def update_graph(s_date, e_date, category, location, region, chk_day, hist_rt, n):
        ...
        if (STATUS['st_date'] != start_date) or (STATUS['ed_date'] != finish_date):
            # city_df_30min_345, city_df_30min_45 = df_pick(region)
            city_df_30min_345, city_df_30min_45 = query_transf(start_date.day, finish_date.day, start_date.month,
                                                               start_date.year)
        ...
        if hist_rt == "HD":
            if category == '3-4-5':
                ...
                if location == None:
                    figb = px.scatter_mapbox(lat=[coords[coords['region'] == region].city_la.iloc[0]],
                                             lon=[coords[coords['region'] == region].city_lo.iloc[0]], zoom=3,
                                             height=500)
                    figb.update_layout(mapbox_style="carto-positron", mapbox_zoom=12,
                                       mapbox_center_lat=coords[coords['region'] == region].mpbx_center.iloc[0],
                    ...
                    if chk_day == 'ALL':
                        df55 = city_df_30min_345[(city_df_30min_345['30min'].between(start_date, finish_date)) & (
                                city_df_30min_345['new_street'] == location)].drop_duplicates(
                            subset=['line_geojson'], keep='first').copy()
                    elif chk_day == 'BD':
                        df55 = city_df_30min_345[(city_df_30min_345['30min'].between(start_date, finish_date)) & (
                                city_df_30min_345['new_street'] == location) & (
                                                         city_df_30min_345['weekday'] == 'BD')].drop_duplicates(
                            subset=['line_geojson'], keep='first').copy()
                    else:
                        df55 = city_df_30min_345[(city_df_30min_345['30min'].between(start_date, finish_date)) & (
                                city_df_30min_345['new_street'] == location) & (
                                                         city_df_30min_345['weekday'] == 'FD')].drop_duplicates(
                            subset=['line_geojson'], keep='first').copy()

                    lats, lons = linestr(df55)
                    dff = pd.DataFrame({'lat': lats, 'lon': lons})

                    figb = px.line_mapbox(dff, lat='lat', lon='lon', color_discrete_sequence=['red'], zoom=3,
                                          height=500)
                    figb.update_layout(mapbox_style="carto-positron", mapbox_zoom=12,
                                       mapbox_center_lat=dff['lat'].mean(),
                                       margin={"r": 0, "t": 0, "l": 0, "b": 0})
            else:
                if location == None:
                    figb = px.scatter_mapbox(lat=[coords[coords['region'] == region].city_la.iloc[0]],
                    ...
                    if chk_day == 'ALL':
                        df55 = city_df_30min_45[(city_df_30min_45['30min'].between(start_date, finish_date)) & (
                                city_df_30min_45['new_street'] == location)].drop_duplicates(
                            subset=['line_geojson'], keep='first').copy()
                    elif chk_day == 'BD':
                        df55 = city_df_30min_45[(city_df_30min_45['30min'].between(start_date, finish_date)) & (
                                city_df_30min_45['new_street'] == location) & (
                                                        city_df_30min_45['weekday'] == 'BD')].drop_duplicates(
                            subset=['line_geojson'], keep='first').copy()
                    else:
                        df55 = city_df_30min_45[(city_df_30min_45['30min'].between(start_date, finish_date)) & (
                                city_df_30min_45['new_street'] == location) & (
                                                        city_df_30min_45['weekday'] == 'FD')].drop_duplicates(
                            subset=['line_geojson'], keep='first').copy()
            ...
            map_live = pd.read_csv(args.map)
            if category == '3-4-5':
                figb = px.line_mapbox(map_live, lat='latitude', lon='longitude', color_discrete_sequence=['red'],
                                      zoom=2, height=500)
                figb.update_layout(mapbox_style="carto-positron", margin={"r": 0, "t": 0, "l": 0, "b": 0},
                                   mapbox_zoom=12, mapbox_center_lat=map_live['latitude'].mean(), uirevision=True)
            elif category == '4-5':
                figb = px.line_mapbox(map_live[map_live['level'] != 3], lat='latitude', lon='longitude',
                                      color_discrete_sequence=['red'], zoom=2, height=500)
                figb.update_layout(mapbox_style="carto-positron", margin={"r": 0, "t": 0, "l": 0, "b": 0},
                                   mapbox_zoom=12, mapbox_center_lat=map_live[map_live['level'] != 3].latitude.mean(),
                                   uirevision=True)
        ...
        if hist_rt == "HD":
            if category == '3-4-5':
                if location == None:
                    ...
                    if chk_day == 'ALL':
                        ...
                        df1 = city_df_30min_345[
                            city_df_30min_345['30min'].between(start_date, finish_date)].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df4 = df3[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()

                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_345[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()

                    elif chk_day == 'BD':
                        # base filtrada pela data
                        df1 = city_df_30min_345[(city_df_30min_345['30min'].between(start_date, finish_date)) & (
                                city_df_30min_345['weekday'] == 'BD')].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_345[['30min', 'new_street', 'length', 'time_hm']][
                            city_df_30min_345['weekday'] == 'BD'].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()
                        ...
                        # base filtrada pela data
                        df1 = city_df_30min_345[(city_df_30min_345['30min'].between(start_date, finish_date)) & (
                                city_df_30min_345['weekday'] == 'FD')].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_345[['30min', 'new_street', 'length', 'time_hm']][
                            city_df_30min_345['weekday'] == 'FD'].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()
                    ...
                    if chk_day == 'ALL':
                        # base filtrada pela data
                        df1 = city_df_30min_345[city_df_30min_345['30min'].between(start_date, finish_date) & (
                                city_df_30min_345['new_street'] == location)].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_345[['30min', 'new_street', 'length', 'time_hm']][
                            city_df_30min_345['new_street'] == location].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()
                        ...
                        # base filtrada pela data
                        df1 = city_df_30min_345[(city_df_30min_345['30min'].between(start_date, finish_date)) & (
                                city_df_30min_345['weekday'] == 'BD') & (city_df_30min_345[
                                                                             'new_street'] == location)].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # base filtrada pela data
                        df1 = city_df_30min_345[(city_df_30min_345['30min'].between(start_date, finish_date)) & (
                                city_df_30min_345['weekday'] == 'FD') & (city_df_30min_345[
                                                                             'new_street'] == location)].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                ...
                if location == None:
                    if chk_day == 'ALL':
                        # base filtrada pela data
                        df1 = city_df_30min_45[
                            city_df_30min_45['30min'].between(start_date, finish_date)].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_45[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()
                        ...
                        # base filtrada pela data
                        df1 = city_df_30min_45[(city_df_30min_45['30min'].between(start_date, finish_date)) & (
                                city_df_30min_45['weekday'] == 'BD')].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_45[['30min', 'new_street', 'length', 'time_hm']][
                            city_df_30min_45['weekday'] == 'BD'].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()
                        ...
                        # base filtrada pela data
                        df1 = city_df_30min_45[(city_df_30min_45['30min'].between(start_date, finish_date)) & (
                                city_df_30min_45['weekday'] == 'FD')].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_45[['30min', 'new_street', 'length', 'time_hm']][
                            city_df_30min_45['weekday'] == 'FD'].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()
                    ...
                    if chk_day == 'ALL':
                        # base filtrada pela data
                        df1 = city_df_30min_45[city_df_30min_45['30min'].between(start_date, finish_date) & (
                                city_df_30min_45['new_street'] == location)].dropna().copy()
                        # obtendo a soma de congestionamento por segmento
                        df2 = df1[['30min', 'new_street', 'length', 'time_hm']].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df3 = df2[['time_hm', 'new_street', 'length']].groupby(
                        ...
                        # obtendo a soma de congestionamento por segmento
                        df6 = city_df_30min_45[['30min', 'new_street', 'length', 'time_hm']][
                            city_df_30min_45['new_street'] == location].groupby(
                            ['30min', 'new_street', 'time_hm']).sum().reset_index()
                        df7 = df6[['time_hm', 'new_street', 'length']].groupby(
                            ['time_hm', 'new_street']).mean().reset_index()
                        df8 = df7[['time_hm', 'length']].groupby(['time_hm']).sum().reset_index()
                    ...
                    fig.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)', 'paper_bgcolor': 'rgba(0, 0, 0, 0)', },
                                      template="plotly_dark", uirevision=True)
                    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgb(102,102,102)')
            ...
            fig.add_hline(y=mean_global, line_width=2, line_color="white", annotation_text="Mean baseline",
                          annotation_position="bottom right")
            fig.add_hline(y=mean_global + std_global, line_width=1, line_dash="dash", line_color="white",
                          annotation_text="Mean + 1 std", annotation_position="bottom right",
                          annotation_font_color="white")
            fig.add_hline(y=mean_global - std_global, line_width=1, line_dash="dash", line_color="white",
                          annotation_text="Mean - 1 std", annotation_position="bottom right",
                          annotation_font_color="white")
            fig.add_hline(y=mean_local, line_width=3, line_color="yellow", annotation_text="Mean",
            ...
            fig_a.add_trace(go.Indicator(
                ...
                value=mean_local,
                title={
                    "text": "Traffic Congestion Indicator<br><span style='font-size:0.8em;color:gray'>Traffic congestion in km</span><br><span style='font-size:0.8em;color:gray'>% Last update vs Baseline</span>"},
                delta={'reference': mean_global, 'relative': True},
                domain={'x': [0, 1], 'y': [0, 1]}))
            fig_a.update_layout(paper_bgcolor="#1e1e1e", width=450, height=310)
            fig_a.update_traces(title_font_color="white", delta_increasing_color="#FB0D0D",
                                delta_decreasing_color="#00FE35", number_font_color="white")

        else:
            sp_df_5min = pd.read_csv(argparse.traffic_now, parse_dates=['5min'])
            sp_df_5min['length'] = sp_df_5min['length'] / 1000
            if category == '3-4-5':
                # base filtrada pela data
                df1 = sp_df_5min[sp_df_5min['5min'] >= dt.date.today()].dropna().copy()
                # obtendo a soma de congestionamento por segmento
                df2 = df1[['5min', 'new_street', 'length', 'time_hm']].groupby(
                    ['5min', 'new_street', 'time_hm']).sum().reset_index()
                ...
                fig.add_trace(go.Scatter(x=hist_345['time_hm'], y=hist_345['mean_p10'], name='Baseline + 10%',
                                         line=dict(color='#19D3F3', width=2, dash='dash')))
                fig.add_trace(go.Scatter(x=hist_345['time_hm'], y=hist_345['mean_m10'], name='Baseline - 10%',
                                         line=dict(color='#0099C6', width=2, dash='dash')))
                ...
                fig.update_layout({'plot_bgcolor': 'rgba(0, 0, 0, 0)', 'paper_bgcolor': 'rgba(0, 0, 0, 0)', },
                                  template="plotly_dark", uirevision=True, xaxis_title="Time interval [5 min]",
                                  yaxis_title="Traffic congestion [km]")
                fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgb(102,102,102)')
                ...
                fig_a.add_trace(go.Indicator(
                    ...
                    title={
                        "text": "Traffic Congestion Indicator<br><span style='font-size:0.8em;color:gray'>Traffic congestion in km</span><br><span style='font-size:0.8em;color:gray'>% variation Last update vs Baseline</span>"},
                    delta={'reference': np.float(hist_345[hist_45['time_hm'] == np.max(df4['time_hm'])]['mean']),
                           'relative': True},
                    domain={'x': [0, 1], 'y': [0, 1]}))
                fig_a.update_layout(paper_bgcolor="#1e1e1e", width=450, height=310)
                fig_a.update_traces(title_font_color="white", delta_increasing_color="#FB0D0D",
                ...
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=hist_45['time_hm'], y=hist_45['mean'], name='Baseline',
                                         line=dict(color='white', width=2)))
                fig.add_trace(go.Scatter(x=hist_45['time_hm'], y=hist_45['mean_p10'], name='Baseline + 10%',
                                         line=dict(color='#19D3F3', width=2, dash='dash')))
                fig.add_trace(go.Scatter(x=hist_45['time_hm'], y=hist_45['mean_m10'], name='Baseline - 10%',
                                         line=dict(color='#0099C6', width=2, dash='dash')))
                fig.add_bar(x=df4['time_hm'], y=df4['length'], name='Last 5 min')
                # Add shape regions
                fig.add_vrect(
                ...
                fig_a.add_trace(go.Indicator(
                    ...
                    value=np.float(df4[df4['time_hm'] == np.max(df4['time_hm'])].length),
                    title={
                        "text": "Traffic Congestion Indicator<br><span style='font-size:0.8em;color:gray'>Traffic congestion in km</span><br><span style='font-size:0.8em;color:gray'>% variation Mean vs Mean baseline</span>"},
                    delta={'reference': np.float(hist_45[hist_45['time_hm'] == np.max(df4['time_hm'])]['mean']),
                           'relative': True},
                    domain={'x': [0, 1], 'y': [0, 1]}))
                fig_a.update_layout(paper_bgcolor="#1e1e1e", width=450, height=310)
                fig_a.update_traces(title_font_color="white", delta_increasing_color="#FB0D0D",
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--option_list",
        type=str,
        default="../data/option_list",
        help="FROM-TO list, Waze segments for city mapping.",
    )

    parser.add_argument(
        "--map",
        type=str,
        default="../data/map.csv",
        help="Map list.",
    )
    parser.add_argument(
        "--traffic_now",
        type=str,
        default="../data/traffic_now.csv",
        help="Traffic now.",
    )
