...
# arquivos de DE PARA
de_para_sp = pd.read_csv('../data/sp_streets.txt')
...
while True:
    ...
    for i in cities:
        ...
        for j in range(len(data['jams'])):
            try:
                if (data['jams'][j]['city'] == 'São Paulo') & (data['jams'][j]['level'] >= 3) & (
                        data['jams'][j]['street'] in list(de_para_sp.street)):
                    print('***************************************')
                    ...
                    timestamp = np.append(timestamp, dt.datetime.strptime(data['endTime'][:-4],
                                                                          '%Y-%m-%d %H:%M:%S') - dt.timedelta(hours=3))
                    for k in range(len(data['jams'][j]['line'])):
                        ...
                        timestamp_map = np.append(timestamp_map, dt.datetime.strptime(data['endTime'][:-4],
                ...
                continue
        if i == 'Sao Paulo 1':
            sp1_data = Maplatlong(latitude_map, longitude_map, street, timestamp, level, length, timestamp_map,
                                  level_map)
        elif i == 'Sao Paulo 2':
            sp2_data = Maplatlong(latitude_map, longitude_map, street, timestamp, level, length, timestamp_map,
                                  level_map)
        elif i == 'Sao Paulo 3':
            sp3_data = Maplatlong(latitude_map, longitude_map, street, timestamp, level, length, timestamp_map,
                                  level_map)
    ...
    df_map.to_csv('../data/map.csv', header=True, index=False)

    df1_hist = pd.DataFrame({"timestamp": sp1_data.timestamp,
                             "new_street": sp1_data.street,
                             "level": sp1_data.level, "length": sp1_data.length})
    df2_hist = pd.DataFrame({"timestamp": sp2_data.timestamp, "new_street": sp2_data.street, "level": sp2_data.level,
                             "length": sp2_data.length})
    df3_hist = pd.DataFrame({"timestamp": sp3_data.timestamp, "new_street": sp3_data.street, "level": sp3_data.level,
                             "length": sp3_data.length})
    ...
    df_hist.to_csv('../data/traffic_now.csv', mode='a', header=False, index=False)
